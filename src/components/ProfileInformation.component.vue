  ...
  <section class="w-full container-section">
    <div class="flex flex-row items-center -translate-y-14 -mb-5">
      <figure class="inline-flex bg-[#20293A] rounded-3xl p-2.5">
        <div class="inline-flex h-full rounded-2xl bg-slate-900">
          <img :src="userInfo?.avatar_url" class="w-36 rounded-full">
        </div>
      </figure>
    ...
    <div class="flex flex-row items-center justify-between">
      <div class="flex flex-col gap-4">
        <h2 class="text-4xl font-medium text-[#CDD5E0]">{{ userInfo?.login }}</h2>
        <p class="text-lg font-light text-[#CDD5E0]/70">{{ userInfo?.bio }}</p>
      </div>
      <div class="p-4 bg-[#111729] rounded-xl">
        <p class="text-[#4A5567] text-lg">Total Repositories: <span class="text-[#CDD5E0] ml-2">{{ userRepos?.length }}</span></p>
      </div>
    </div>
